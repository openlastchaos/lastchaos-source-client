...
void CUIFlowerTree::Render()
{
	...
	if (!m_vecSvScoreList.empty())
	{
		...
		for (SvScItr=m_vecSvScoreList.begin(); SvScItr!=m_vecSvScoreList.end(); SvScItr++)
		{
			_pUIMgr->GetDrawPort()->PutTextEx( (*SvScItr).strServerName, m_nPosX+17, nY, 0xFFFFFFFF); // 서버 이름
			
			strScore.PrintF("%d", (*SvScItr).ulScore); 
			_pUIMgr->GetDrawPort()->PutTextExRX(strScore, testX+145, nY, 0xFFFFFFFF); // 서버 점수
			nY += 15;
		}
	}

	_pUIMgr->GetDrawPort()->PutTextEx( _S(3486, "접속 서버 :"), m_nPosX+17, m_nPosY+FLOWERTREE_HEIGHT-70, 0xFFFFFFFF);
...
void CUIFlowerTree::FlowerTreeUpdate(ULONG ulPoint)
{
	...
	if ( m_MobFlowerTree.mob_iClientIndex < 0 ) return; // 꽃나무 정보가 없다.
	...
	if (m_MobFlowerTree.mob_iType == MOB_FLOWERTREE_INDEX) // 꽃나무
	{
